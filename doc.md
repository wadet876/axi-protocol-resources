...
This section has useful diagrams for making state machines. Please see the original spec as the info are straightforward.

### Snoop Channels
I will be skipping snoop channels because we will not be using AXI as a way to maintain and sync a multilevel memory system. Just know that that is something supported in AXI5 using the addition of another channel.
...
This is to say: 
- Manager issues the first address only even if the transaction is a burst.
- Subordinate calculate which addresses' data to send in accordance with the first address and burst parameter from manager
- No burst is allowed to cross a 4KB boundary.
	- Since many memory systems use 4KB pages/blocks, this rule simplifies interconnect and subordinate memory region designs
	- This also means that a burst cannot be larger than 4KB of data
	-  Example: Burst start at 0x0FFC, and you asked for a 4‑beat INCR burst of 4‑byte transfers, the subordinate would see that the fourth beat would land at 0x100Cw, hich crosses from the “0x0000–0x0FFF” region into “0x1000–0x1FFF.” That’s disallowed.
- For WRAP burst types, the “wrap boundary” is always a multiple of the total burst size. Guaranteeing that (Size × Length) ≤ 4 KB means those arithmetic calculations stay within a fixed, small range, again keeping the hardware simple.
- 
For our purpose, the memory system is unpaged but keeping the 4KB limitation in mind is a generally good idea.
